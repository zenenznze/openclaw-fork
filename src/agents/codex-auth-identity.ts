...
export function resolveCodexStableSubject(params: {
  payload: CodexJwtPayload | null;
  accountId?: string | null;
}): string | undefined {
  const auth = params.payload?.["https://api.openai.com/auth"];
  const accountUserId =
    normalizeNonEmptyString(auth?.chatgpt_account_user_id) ??
    normalizeNonEmptyString(params.accountId);
  if (accountUserId) {
  ...
  const iss = normalizeNonEmptyString(params.payload?.iss);
  const sub = normalizeNonEmptyString(params.payload?.sub);
  if (iss && sub) {
    return `${iss}|${sub}`;
  }
  return sub;
...
export function resolveCodexAuthIdentity(params: {
  ...
  const stableSubject = resolveCodexStableSubject({ payload, accountId: params.accountId });
  if (!stableSubject) {
    return {};
  }
